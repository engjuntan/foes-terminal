...
// --- PEOPLE (job 5, SCOPE_DECISIONS.md "People tab" ruling: "People
// notes are browsed inside the Data Logs tab") — identical grant/unlock/
// read shape to Data Logs above, just against `characters.<id>.unlocked_people`
// / `read_people` and the vault's own `peopleDatabase` (src/people.js,
// generated by sync-obsidian.js from People/ notes) instead of
// `unlocked_logs` / `dataLogDatabase`. Kept as its own pair of functions
// rather than parameterizing openDataLog/gmGrantDataLog — the Firestore
// field name differs per call, and duplicating ~10 lines reads clearer
// than threading a field-name string through every call site.
export async function gmGrantPerson(personId, target) {
  const characters = window.liveData.characters || {};
  const targets = target === 'all' ? Object.keys(characters).filter(id => characters[id].is_finalized) : [target];
  const updatePayload = {};
  targets.forEach(charId => {
    const current = characters[charId].unlocked_people || [];
    if (!current.includes(personId)) updatePayload[`characters.${charId}.unlocked_people`] = [...current, personId];
  });
...
export async function openPerson(personId) {
  window.openPersonId = window.openPersonId === personId ? null : personId;
  window.render();
  const char = window.liveData.characters[window.currentUser];
  const readPeople = char.read_people || [];
  if (!readPeople.includes(personId)) {
    const charRef = doc(db, "prisoncampaign", "alpha_team");
    const updatePayload = {};
    updatePayload[`characters.${window.currentUser}.read_people`] = [...readPeople, personId];
    try { await updateDoc(charRef, updatePayload); } catch (err) { alert("ERROR: " + err.message); }
  }
}
